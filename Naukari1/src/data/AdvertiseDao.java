...
public class AdvertiseDao {
	...
	public boolean submitAdd(Advertise add) throws SQLException, NoSuchFieldException, SecurityException,
		...
		pst.setDate(9, new Date(util.DateUtil.getHtmlToJavaDate(add.getStartDate()).getTime()));
		pst.setDate(10, new Date(util.DateUtil.getHtmlToJavaDate(add.getEndDate()).getTime()));
		...
		pst.setDate(18, new Date(new java.util.Date().getTime()));
	...
	public List<Advertise> getAdevertise() throws Exception {
		Advertise ad = null;
		 ls = new ArrayList<>();
		con = DBConnection.getConnection();
		pst = con.prepareStatement(ALL_LIST);
	...
	public Advertise getAddById(String id) throws Exception {
		...
		Advertise ad = null;
		con = DBConnection.getConnection();
		pst = con.prepareStatement(ADD_BY_ID);
		pst.setString(1,id);
		ResultSet rs = pst.executeQuery();
		if(rs.next()) {
			ad = new Advertise();
